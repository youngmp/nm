...
def rhs(t, z, pdict, option='value',idx=''):
    ...
    Wilson and Ermentrout RSTA 2019 ... not exactly
    note that this version exhibits limit cycles in phase differences for 1:1 for eps=.025 or so.
    ...
    if option == 'value' or option == 'val':
        return np.array([x*(1-R2)-q*y*R2,y*(1-R2)+q*x*R2])
    elif option == 'sym' or option == 'symbolic':
        return Matrix([x*(1-R2)-q*y*R2,y*(1-R2)+q*x*R2])


def rhs_old2(t,z,pdict,option='value',idx=''):
    """
    Right-hand side of the Complex Ginzburgh-Landau (CGL) model from
